// AesTunSvr.cpp : 此文件包含 "main" 函数。程序执行将在此处开始并结束。
//
...
int SvrPostAccept(CLfrpSocket* pSocket)
{
    if (pSocket == nullptr)
        return -1;
    ...
    if (pSocket)
    {
        ...
        if (iter != pSocketPairMapAry[nIndex].end())
        {
            ...
            { // 收到请求
                ...
                AesTunRecvAndMoveDate(pSocket, pair.pServer, pBuffer, nCount, pair);
            }
            ...
            { // 收到回复
                ...
                AesTunMakeSendPack(pair.pVistor, pBuffer, nCount, nSeq);
...
{
    int nRet = 0;
    PRINT_ERROR("%s Svr used as 'AesTunSvr -p ListenPort -sh ServerHost -sp ServerPort -k AESKey', default is 'AesTunSvr -p %d -sh %s -sp %d -k %s'\n", GetCurTimeStr(), nPort, strSvrHost, nSvrPort, strAesKey.c_str());
    int i = 0;
    for (i = 0; i < argc; i++)
    {
    ...
    InitLog("./Svr.txt");
    // 初始化epooll工作线程相关
    SetBusWorkerCallBack(SvrRead, SvrWrite, SvrClose, SvrTrans, SvrPostAccept, SvrTimer);
    InitWorkerThreads();
