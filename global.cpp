...
void AesTunSendDate(CLfrpSocket* pSocket, bool bEncrpyt, bool bSendHeader, CSocketPair& pair)
{
    ...
    for (int i = 0; i < pSocket->vecSendBuf.size(); i++)
    {
        ...
        // 如下方式会卡住当前线程，通过bLastEAGAIN和cLastSendBuf转掉可以使用epoll的机制
        // int nHasSend = 0;
        // while (nRealSendLen > 0)
        // {
        //     nRet = send(pSocket->sock, pRealSendData + nHasSend, nRealSendLen, LFRP_SEND_FLAGS);
        ...
        { // 堵住就等下一个EPOLLOUT事件，清掉已经发送的数据
            PRINT_INFO("%s %s,%d: send err sock %d sockID %d size %d sendHeader %d wsaerr WSAEWOULDBLOCK %d\n", \
                        GetCurTimeStr(), __FUNCTION__, __LINE__, pSocket->sock, pSocket->nSocketID, nSendLen, bSendHeader, WSAGetLastError());
            nError = true;
            CVecBuffer& vecBuf = pSocket->vecSendBuf;
        ...
        if (nRet > 0 && nRet < nRealSendLen)
        {
            PRINT_INFO("%s %s,%d: send err sock %d sockID %d size %d sendHeader %d wsaerr WSAEWOULDBLOCK pre %d send %d\n", \
                        GetCurTimeStr(), __FUNCTION__, __LINE__, pSocket->sock, pSocket->nSocketID, nSendLen, bSendHeader, nRealSendLen, nRet);
            char* pNextBuf = new char[nRealSendLen - nRet];
            memcpy(pNextBuf, pRealSendData + nRet, nRealSendLen - nRet);
            SetCBuf(pSocket->cLastSendBuf, pNextBuf, nRealSendLen - nRet);
#ifdef USE_AES            
            if (bEncrpyt)
            {
                delete[] pSendBuffer;
...
#endif 
            CVecBuffer& vecBuf = pSocket->vecSendBuf;
            DeleteBufItems(pSocket->vecSendBuf, i + 1);   // 当前包发了一部分，vec里存下一个包开始
