...
#define RECV_BUFFER_SIZE    1024    //10*1024        // socket接受一次大小
...
public:
    ...
    CBuffer cLastSendBuf;       // 上一个发送包
    int nLastPreSendSize;       // 上一次准备发送的大小
    int nLastRealSendSize;      // 上一次实际发送的大小
    int bLastEAGAIN;            // 上一次发送堵塞，等待下一次发送时，先发上一个包
    int nLastRecvSeq;           // 最后收到的Seq，用于去重 
