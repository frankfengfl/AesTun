...
void EpollReadET(int nIndex, int sockfd, fnRead cbRead, fnClose cbClose)
{
    ...
    { // et模式一直读到EAGAIN
        ...
        PRINT_INFO("%s %s,%d: EpollReadET thread %d recv sock %d ret %d\n", GetCurTimeStr(), __FUNCTION__, __LINE__, nIndex, sockfd, ret);
        if (ret < 0)
        {
            PRINT_INFO("%s %s,%d: EpollReadET thread %d recv sock %d ret %d err %d\n", GetCurTimeStr(), __FUNCTION__, __LINE__, nIndex, sockfd, ret, errno);
            if (errno == EAGAIN || errno == EWOULDBLOCK || errno == EINTR)
            {
                break;
            }
...
int GetThreadIndex(int sock)
{
    ...
    if (pSocket)
    {
        nThreadIndex = GetThreadIndexByNum(pSocket->nServiceNumber);
    }
    return nThreadIndex;
}
